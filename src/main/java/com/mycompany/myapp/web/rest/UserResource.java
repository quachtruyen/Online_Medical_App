...
import com.mycompany.myapp.repository.UserRepositoryCustom;
...
public class UserResource {
    ...
    public ResponseEntity<User> createUser(@Valid @RequestBody UserInfoDTO userDTO) throws URISyntaxException {
    ...
    @PutMapping("/users")
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<UserInfoDTO> updateUser(@Valid @RequestBody UserInfoDTO userDTO) {
        ...
        Optional<UserInfoDTO> updatedUser = userService.updateUser(userDTO);
    ...
    @GetMapping("/users")
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<List<UserInfoDTO>> getAllUsers(Pageable pageable, @RequestParam String authorities) { // pagable
        // là
        // đối
        // tượng
        // để
        ...
        final Page<UserInfoDTO> page;
        if (StringUtils.isBlank(authorities)) {
            page = userService.getAllManagedUsers(pageable);
        } else {
            page = userService.getAllManagedUsersAndAuthor(pageable, authorities);
        }
    ...
    @GetMapping("/patients")
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<List<UserPatientDTO>> getAllPatient(
        Pageable pageable,
        @RequestParam String healthStatus,
        @RequestParam String patientStatus
    ) { // pagable
        List<UserPatientDTO> userInfoDTOS = userRepositoryCustom.findAllPatients(
            null,
            healthStatus,
            patientStatus,
        ...
        Integer total = userRepositoryCustom.countAllPatients(null, healthStatus, patientStatus);
        HttpHeaders headers = new HttpHeaders();
        headers.set("X-Total-Count", total.toString());
        return new ResponseEntity<>(userInfoDTOS, headers, HttpStatus.OK);
    }
    ...
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<UserInfoDTO> getUser(@PathVariable @Pattern(regexp = Constants.LOGIN_REGEX) String login) {
        log.debug("REST request to get User : {}", login);
        return ResponseUtil.wrapOrNotFound(Optional.of(userService.getUserWithAuthoritiesByLogin(login)));
    }
